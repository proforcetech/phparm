...
- Actual fix: Closed the immediate exploit windows without yet swapping in a real upload pipeline. (1) `VoiceNoteService::record()` now rejects absolute paths (POSIX `/`, Windows `C:\`, even on a Linux host — the data may have been authored elsewhere) and null bytes, in addition to the existing `..` check. (2) `HeuristicTranscriber` now requires a non-empty `storageRoot`; the empty-root mode is gated behind an explicit `allowEmptyRootForTests: true` flag so production wiring can no longer silently fall into the read-anywhere-from-CWD branch. (3) `HeuristicTranscriber::resolve()` canonicalises both the storage root and the candidate's parent directory with `realpath()` and refuses to read a sidecar whose canonical parent does not live under the canonical root — defeats symlink escapes that survive the textual checks. (4) `routes/modules/voice_notes.php` wires `storage/private/voice_notes` (auto-created with mode 0750) as the production root.
- Verification: `php tests/VoiceNoteServiceTest.php` — 60 tests pass, including 6 new cases: `heuristic_constructor_rejects_empty_root_in_production`, `heuristic_refuses_absolute_path`, `heuristic_refuses_null_byte`, `record_rejects_absolute_unix_path`, `record_rejects_absolute_windows_path`, `record_rejects_null_byte_in_path`.
- Residual risk: The architectural change recommended in the original Recommended-fix line — replacing the client-supplied `audio_path` with a server-generated path off a multipart upload — is **deferred to `audit-v2-recommendations.md`**. Until that lands, an authenticated `voice_notes.create` actor can still pick the *relative* sub-path their audio lives at within the storage root, which permits collisions with other tenants' notes (an integrity issue, not a confidentiality one — the canonicalisation guard means they can't read outside the root). The path-pattern exposure in transcripts/UIs is an attack surface that should be removed entirely by the upload-pipeline rewrite.
- Re-verified: 2026-05-10 (Phase 2 fix — partial)
