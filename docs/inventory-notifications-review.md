...
1. **Add alerts page and menu entry**
   - Create a low-stock alerts view with filtering, pagination, and edit navigation, and link it from the sidebar/dashboard tiles.
   - Implement an API route/controller handler for low-stock listings using `InventoryItemController::lowStock` and gate checks.
...
2. **Expose actionable dashboard tile**
   - Render a dashboard widget/tile that shows low/out-of-stock counts with a CTA to the alerts page and optional preview of top items.
   - Reuse `InventoryLowStockService::tile` data to keep counts consistent across widgets and alerts pages.
...
3. **Schedule and template email alerts**
   - Configure a scheduled job or queue worker to invoke `sendEmailAlert` with tenant-specific recipients and subject lines.
   - Add notification template entries and translations for `inventory.low_stock_alert`, ensuring payload matches email markup expectations.
