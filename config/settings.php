<?php

return [
    'defaults' => [
        'shop.name' => [
            'group' => 'profile',
            'type' => 'string',
            'description' => 'Display name used across invoices, estimates, and the customer portal.',
            'value' => null,
        ],
        'shop.email' => [
            'group' => 'profile',
            'type' => 'string',
            'description' => 'Primary contact email for notifications and document headers.',
            'value' => null,
        ],
        'shop.phone' => [
            'group' => 'profile',
            'type' => 'string',
            'description' => 'Primary phone number rendered on outbound documents.',
            'value' => null,
        ],
        'shop.logo_url' => [
            'group' => 'profile',
            'type' => 'string',
            'description' => 'Public URL for the shop logo used on estimates, invoices, and portal pages.',
            'value' => null,
        ],
        'shop.address' => [
            'group' => 'profile',
            'type' => 'json',
            'description' => 'Address payload containing street, city, state, postal code, and country.',
            'value' => [
                'street' => null,
                'city' => null,
                'state' => null,
                'postal_code' => null,
                'country' => null,
            ],
        ],
        'pricing.tax_rate' => [
            'group' => 'pricing',
            'type' => 'float',
            'description' => 'Default sales tax percentage applied to taxable line items.',
            'value' => 0.0,
        ],
        'pricing.labor_rate' => [
            'group' => 'pricing',
            'type' => 'float',
            'description' => 'Default hourly labor rate applied when creating estimate labor lines.',
            'value' => 0.0,
        ],
        'pricing.call_out_fee' => [
            'group' => 'pricing',
            'type' => 'float',
            'description' => 'Default call-out or mobile service fee.',
            'value' => 0.0,
        ],
        'pricing.mileage_rate' => [
            'group' => 'pricing',
            'type' => 'float',
            'description' => 'Mileage charge per unit used for mobile appointments.',
            'value' => 0.0,
        ],
        'pricing.labor_taxable' => [
            'group' => 'pricing',
            'type' => 'boolean',
            'description' => 'Whether labor charges are taxable by default.',
            'value' => false,
        ],
        'pricing.fee_taxable' => [
            'group' => 'pricing',
            'type' => 'boolean',
            'description' => 'Whether fees are taxable by default.',
            'value' => false,
        ],
        'integrations.stripe.public_key' => [
            'group' => 'integrations',
            'type' => 'string',
            'description' => 'Stripe publishable API key used for payment forms.',
            'value' => null,
        ],
        'integrations.stripe.secret_key' => [
            'group' => 'integrations',
            'type' => 'string',
            'description' => 'Stripe secret API key used for server-side payment actions.',
            'value' => null,
        ],
        'integrations.stripe.webhook_secret' => [
            'group' => 'integrations',
            'type' => 'string',
            'description' => 'Stripe webhook signing secret for verifying incoming payment events.',
            'value' => null,
        ],
        'integrations.square.token' => [
            'group' => 'integrations',
            'type' => 'string',
            'description' => 'Square access token for payments and catalog sync.',
            'value' => null,
        ],
        'integrations.square.webhook_signature_key' => [
            'group' => 'integrations',
            'type' => 'string',
            'description' => 'Square webhook signature key for validating notifications.',
            'value' => null,
        ],
        'integrations.paypal.client_id' => [
            'group' => 'integrations',
            'type' => 'string',
            'description' => 'PayPal client ID for web checkout flows.',
            'value' => null,
        ],
        'integrations.paypal.client_secret' => [
            'group' => 'integrations',
            'type' => 'string',
            'description' => 'PayPal client secret for token exchange.',
            'value' => null,
        ],
        'integrations.paypal.webhook_id' => [
            'group' => 'integrations',
            'type' => 'string',
            'description' => 'PayPal webhook ID used to validate incoming payment webhooks.',
            'value' => null,
        ],
        'integrations.zoho.client_id' => [
            'group' => 'integrations',
            'type' => 'string',
            'description' => 'Zoho CRM client ID for syncing customer or invoice records.',
            'value' => null,
        ],
        'integrations.zoho.client_secret' => [
            'group' => 'integrations',
            'type' => 'string',
            'description' => 'Zoho CRM client secret for token exchange.',
            'value' => null,
        ],
        'integrations.zoho.refresh_token' => [
            'group' => 'integrations',
            'type' => 'string',
            'description' => 'Zoho CRM refresh token used to rotate access tokens.',
            'value' => null,
        ],
        'integrations.zoho.org_id' => [
            'group' => 'integrations',
            'type' => 'string',
            'description' => 'Zoho CRM organization ID for API requests.',
            'value' => null,
        ],
        'notifications.mail.from_name' => [
            'group' => 'notifications',
            'type' => 'string',
            'description' => 'Default email from-name for transactional emails.',
            'value' => null,
        ],
        'notifications.mail.from_address' => [
            'group' => 'notifications',
            'type' => 'string',
            'description' => 'Default email from-address for transactional emails.',
            'value' => null,
        ],
        'notifications.sms.from_number' => [
            'group' => 'notifications',
            'type' => 'string',
            'description' => 'SMS sender number for appointment, estimate, and invoice messages.',
            'value' => null,
        ],
        'notifications.estimate_request_email' => [
            'group' => 'notifications',
            'type' => 'string',
            'description' => 'Email address to receive notifications for new estimate requests submitted through the public form.',
            'value' => null,
        ],
        'integrations.twilio.sid' => [
            'group' => 'integrations',
            'type' => 'string',
            'description' => 'Twilio Account SID for SMS dispatch.',
            'value' => null,
        ],
        'integrations.twilio.token' => [
            'group' => 'integrations',
            'type' => 'string',
            'description' => 'Twilio auth token for SMS dispatch.',
            'value' => null,
        ],
        'integrations.smtp.host' => [
            'group' => 'integrations',
            'type' => 'string',
            'description' => 'SMTP host name for outbound email.',
            'value' => null,
        ],
        'integrations.smtp.port' => [
            'group' => 'integrations',
            'type' => 'integer',
            'description' => 'SMTP port for outbound email.',
            'value' => 587,
        ],
        'integrations.smtp.username' => [
            'group' => 'integrations',
            'type' => 'string',
            'description' => 'SMTP username for authentication.',
            'value' => null,
        ],
        'integrations.smtp.password' => [
            'group' => 'integrations',
            'type' => 'string',
            'description' => 'SMTP password or token for authentication.',
            'value' => null,
        ],
        'integrations.smtp.encryption' => [
            'group' => 'integrations',
            'type' => 'string',
            'description' => 'SMTP encryption protocol (tls/ssl/none).',
            'value' => 'tls',
        ],
        'integrations.recaptcha.site_key' => [
            'group' => 'integrations',
            'type' => 'string',
            'description' => 'reCAPTCHA site key for public forms.',
            'value' => null,
        ],
        'integrations.recaptcha.secret_key' => [
            'group' => 'integrations',
            'type' => 'string',
            'description' => 'reCAPTCHA secret key for server-side verification.',
            'value' => null,
        ],
        'integrations.recaptcha.enabled' => [
            'group' => 'integrations',
            'type' => 'boolean',
            'description' => 'Enable reCAPTCHA verification for public forms and authentication flows.',
            'value' => false,
        ],
        'integrations.recaptcha.score_threshold' => [
            'group' => 'integrations',
            'type' => 'float',
            'description' => 'Minimum acceptable reCAPTCHA score (0.0 - 1.0) before processing public forms.',
            'value' => 0.5,
        ],
        'security.mandatory_2fa_roles' => [
            'group' => 'security',
            'type' => 'json',
            'description' => 'Roles that must enroll in two-factor authentication before accessing the app.',
            'value' => ['admin', 'dispatcher'],
        ],
        'integrations.partstech.api_base' => [
            'group' => 'integrations',
            'type' => 'string',
            'description' => 'Base URL for PartsTech API requests.',
            'value' => 'https://api.partstech.com',
        ],
        'integrations.partstech.api_key' => [
            'group' => 'integrations',
            'type' => 'string',
            'description' => 'PartsTech API key used for VIN decoding and catalog search.',
            'value' => null,
        ],
        'integrations.partstech.markup_tiers' => [
            'group' => 'integrations',
            'type' => 'json',
            'description' => 'Optional pricing markup tiers applied to PartsTech catalog results.',
            'value' => [],
        ],
        'integrations.bank_feeds.provider' => [
            'group' => 'integrations',
            'type' => 'string',
            'description' => 'Bank feed provider key (e.g., plaid, yodlee, demo).',
            'value' => 'demo',
        ],
        'integrations.bank_feeds.client_id' => [
            'group' => 'integrations',
            'type' => 'string',
            'description' => 'Bank feed provider client ID.',
            'value' => null,
        ],
        'integrations.bank_feeds.client_secret' => [
            'group' => 'integrations',
            'type' => 'string',
            'description' => 'Bank feed provider client secret.',
            'value' => null,
        ],
        'integrations.bank_feeds.access_token' => [
            'group' => 'integrations',
            'type' => 'string',
            'description' => 'Bank feed access token for syncing transactions.',
            'value' => null,
        ],
        'integrations.bank_feeds.status' => [
            'group' => 'integrations',
            'type' => 'string',
            'description' => 'Bank feed connection status.',
            'value' => 'disconnected',
        ],
        'integrations.bank_feeds.last_sync_at' => [
            'group' => 'integrations',
            'type' => 'string',
            'description' => 'Timestamp of the most recent bank feed sync.',
            'value' => null,
        ],
        'integrations.bank_feeds.last_sync_status' => [
            'group' => 'integrations',
            'type' => 'string',
            'description' => 'Status of the most recent bank feed sync.',
            'value' => null,
        ],
        'integrations.bank_feeds.last_sync_count' => [
            'group' => 'integrations',
            'type' => 'integer',
            'description' => 'Number of bank feed transactions synced last run.',
            'value' => 0,
        ],
        'integrations.bank_feeds.last_match_count' => [
            'group' => 'integrations',
            'type' => 'integer',
            'description' => 'Number of ledger matches created during last sync.',
            'value' => 0,
        ],
        'payments.urls.success' => [
            'group' => 'integrations',
            'type' => 'string',
            'description' => 'Redirect URL after successful online payment.',
            'value' => env('APP_URL', 'http://localhost') . '/payment/success',
        ],
        'payments.urls.cancel' => [
            'group' => 'integrations',
            'type' => 'string',
            'description' => 'Redirect URL when a payer cancels checkout.',
            'value' => env('APP_URL', 'http://localhost') . '/payment/cancel',
        ],
        'documents.terms.estimates' => [
            'group' => 'documents',
            'type' => 'string',
            'description' => 'Terms and conditions block rendered on estimates.',
            'value' => null,
        ],
        'documents.terms.invoices' => [
            'group' => 'documents',
            'type' => 'string',
            'description' => 'Terms and conditions block rendered on invoices.',
            'value' => null,
        ],
        'templates.estimate.email_subject' => [
            'group' => 'notifications',
            'type' => 'string',
            'description' => 'Subject line template for estimate sharing emails.',
            'value' => 'Your Estimate #{estimate_number} from {shop_name}',
        ],
        'templates.estimate.email_body' => [
            'group' => 'notifications',
            'type' => 'string',
            'description' => 'HTML body template for estimate sharing emails.',
            'value' => '<p>Hi {customer},</p><p>Your estimate <strong>#{estimate_number}</strong> for <strong>{total}</strong> is ready for review.</p><p>Vehicle: {vehicle}</p><p><a href="{link}">Click here to view your estimate</a></p><p>This estimate expires on {expiration_date}.</p><p>Thank you,<br>{shop_name}<br>{shop_phone}</p>',
        ],
        'templates.estimate.sms_body' => [
            'group' => 'notifications',
            'type' => 'string',
            'description' => 'SMS body template for estimate sharing.',
            'value' => 'Hi {customer}, your estimate #{estimate_number} for {total} is ready. View: {link} - {shop_name}',
        ],
        'storage.notice.notice_of_claim' => [
            'group' => 'storage',
            'type' => 'string',
            'description' => 'HTML template for storage notice of claim PDFs.',
            'value' => null,
        ],
        'storage.notice.lien_notice' => [
            'group' => 'storage',
            'type' => 'string',
            'description' => 'HTML template for storage lien notice PDFs.',
            'value' => null,
        ],
        'storage.notice.tow_authorization' => [
            'group' => 'storage',
            'type' => 'string',
            'description' => 'HTML template for tow authorization forms.',
            'value' => null,
        ],
        'storage.notice.lien_ack' => [
            'group' => 'storage',
            'type' => 'string',
            'description' => 'HTML template for lien acknowledgment forms.',
            'value' => null,
        ],
        'estimates.rejection_reasons' => [
            'group' => 'estimates',
            'type' => 'json',
            'description' => 'List of predefined rejection reasons customers can select when declining an estimate or job.',
            'value' => [
                'Price too high',
                'Found a better deal elsewhere',
                'Decided not to proceed with repairs',
                'Going to a different shop',
                'Vehicle no longer owned',
                'Other'
            ],
        ],
    ],
];
