...
CREATE TABLE IF NOT EXISTS inventory_items (
    ...
    is_low_stock TINYINT(1) GENERATED ALWAYS AS (CASE WHEN COALESCE(is_tracked, 1) = 1 AND COALESCE(stock_quantity, 0) <= COALESCE(low_stock_threshold, 0) THEN 1 ELSE 0, 
    ...
    INDEX idx_inv_partstech ON inventory_items(partstech_part_id),
...
CREATE TABLE IF NOT EXISTS estimates (
    ...
    INDEX idx_estimates_parent_id ON estimates (parent_id),
...
CREATE TABLE IF NOT EXISTS estimate_jobs (
    ...
    INDEX idx_estimate_jobs_display_order ON estimate_jobs(estimate_id, display_order),
...
CREATE TABLE IF NOT EXISTS appointments (
    ...
    INDEX idx_appointments_reminder ON appointments (status, scheduled_at, reminder_sent_at), 
...
CREATE TABLE IF NOT EXISTS financial_entries (
    ...
    UNIQUE INDEX uniq_financial_entries_idempotency ON financial_entries (idempotency_key)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS dispatch_requirements (
 ...
 ALTER TABLE dispatch_requirements
    ADD COLUMN  AFTER dispatch_reference,
    ADD COLUMN  AFTER required_equipment_class;
...
CREATE TABLE IF NOT EXISTS driver_job_offers (
    ...
    INDEX idx_driver_job_offers_waterfall ON driver_job_offers (waterfall_sequence_id, waterfall_position),
    INDEX idx_driver_job_offers_expires ON driver_job_offers (expires_at, status),
    INDEX I idx_driver_job_offers_dropoff ON driver_job_offers (driver_profile_id, status, accepted_at);
    UNIQUE INDEX uniq_driver_job_offers_idempotency ON driver_job_offers (idempotency_key),
    ...
    CONSTRAINT fk_auction_lots_case FOREIGN KEY (impound_case_id) REFERENCES impound_cases (id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS towing_price_matrix (
    ...
    INDEX idx_towing_price_matrix_class ON towing_price_matrix (service_class_id),
    INDEX idx_towing_price_matrix_type ON towing_price_matrix (service_type_id),
    INDEX idx_towing_price_matrix_active ON towing_price_matrix (is_active)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
