...
import vendorPortalTokens from '../../../services/vendor-portal-tokens.service'

/**
 * Phase 18 / C1 — staff-side management page for vendor portal tokens.
 *
 * Pick a vendor, see its outstanding tokens, issue a new one (we surface
 * the plaintext exactly once for copy), or revoke. Issued tokens are
 * scoped to ONE vendor and are the entire credential for the public
 * /vendor-portal/:token page.
 *
 * Read perm: procurement.view; Write perm: procurement.manage
 * (enforced server-side; staff users without those perms get 403s here).
 */
export default function VendorPortalTokens() {
  ...
  useEffect(() => {
    ...
    vendorsService
      ...
      .then((res) => {
        const list = res?.data ?? []
        setVendors(list)
        if (!vendorId && list.length > 0) setVendorId(String(list[0].id))
      })
      .catch((e) => setError(e?.response?.data?.message || e?.message || 'Failed to load vendors'))
  ...
  const submitIssue = async () => {
    ...
    setIssueBusy(true)
    try {
      const res = await vendorPortalTokens.issue(vendorId, {
        label: issueLabel || undefined,
        expires_at: issueExpires || undefined,
      })
      const plaintext = res?.data?.plaintext
      const link = `${window.location.origin}/vendor-portal/${plaintext}`
  ...
  const revoke = async (tokenId) => {
    if (!confirm('Revoke this token? The vendor will lose portal access immediately.')) return
    try {
      await vendorPortalTokens.revoke(tokenId)
      loadTokens()
    } catch (e) {
  ...
  return (
    <div className="space-y-4 p-4">
      <header>
        <h1 className="text-xl font-semibold">Vendor Portal Tokens</h1>
        <p className="text-sm text-gray-500">
          Issue a long-lived bearer token a vendor can use from the public self-service
          portal at <code>/vendor-portal/&lt;token&gt;</code>. The plaintext is shown
      ...
      <Card padding={false}>
        <div className="p-4 flex items-end gap-3 flex-wrap">
          <Select
            ...
            options={[
              { value: '', label: vendorsLoading ? 'Loading…' : 'Select…' },
          ...
          <Button onClick={() => setIssueOpen(true)} disabled={!vendorId}>Issue token</Button>
      ...
      <Modal open={issueOpen} onClose={() => setIssueOpen(false)} title="Issue portal token">
        <div className="space-y-3">
          <Input
            ...
            placeholder="e.g. Acme Parts shipping desk"
          />
          <Input
