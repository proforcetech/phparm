...
/**
 * Phase 18 / C2 — persistence for subcontractor self-service portal tokens.
 *
 ...
 */
class SubcontractorPortalTokenRepository
{
    private const COLUMNS = 'id, subcontractor_id, token_hash, label, expires_at,
    ...
    public function findByPlaintext(string $plaintext): ?SubcontractorPortalToken
    {
        if ($plaintext === '') {
            return null;
        }
        $stmt = $this->connection->pdo()->prepare(
            'SELECT ' . self::COLUMNS . ' FROM subcontractor_portal_tokens
             WHERE token_hash = :h LIMIT 1'
        );
        $stmt->execute(['h' => self::hash($plaintext)]);
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
        return $row ? new SubcontractorPortalToken($row) : null;
    }

    public function findById(int $id): ?SubcontractorPortalToken
    {
        $stmt = $this->connection->pdo()->prepare(
            'SELECT ' . self::COLUMNS . ' FROM subcontractor_portal_tokens
             WHERE id = :id LIMIT 1'
     ...
     */
    public function listForSubcontractor(int $subcontractorId, bool $includeRevoked = false): array
    {
        $where = ['subcontractor_id = :s'];
        if (!$includeRevoked) {
            $where[] = 'revoked_at IS NULL';
        }
        $sql = 'SELECT ' . self::COLUMNS . ' FROM subcontractor_portal_tokens
                WHERE ' . implode(' AND ', $where) . '
                ORDER BY created_at DESC, id DESC LIMIT 100';
        $stmt = $this->connection->pdo()->prepare($sql);
        $stmt->execute(['s' => $subcontractorId]);
        $rows = $stmt->fetchAll(PDO::FETCH_ASSOC) ?: [];
        return array_map(static fn(array $r) => new SubcontractorPortalToken($r), $rows);
    }
    ...
    public function create(
        ...
        ?string $expiresAt,
        ?int $createdByUserId
    ): SubcontractorPortalToken {
        $stmt = $this->connection->pdo()->prepare(
            'INSERT INTO subcontractor_portal_tokens
             (subcontractor_id, token_hash, label, expires_at, created_by_user_id)
             VALUES (:s, :h, :l, :e, :u)'
        );
        $stmt->execute([
            's' => $subcontractorId,
        ...
        if ($found === null) {
            throw new RuntimeException('subcontractor_portal_tokens insert did not return a row');
        }
        return $found;
    }
    ...
    {
        $stmt = $this->connection->pdo()->prepare(
            'UPDATE subcontractor_portal_tokens
