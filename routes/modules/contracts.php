...
return function (Router $router, RouteContext $ctx): void {
    ...
    $router->get('/api/public/contract/view', function (Request $request) use ($signingService) {
    ...
    $router->get('/api/public/contract/by-code/{shortCode}', function (Request $request) use ($signingService) {
        $result = $signingService->fetchPublicView(
            null,
    ...
    $router->post('/api/public/contract/sign', function (Request $request) use ($signingService) {
        $body = $request->body();
        $token = $body['token'] ?? $request->queryParam('token');
        $shortCode = $body['short_code'] ?? $request->queryParam('code');
